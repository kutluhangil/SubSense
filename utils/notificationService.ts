 ...
 */
export const sendRenewalNotifications = (
  subscriptions: Subscription[],
): void => {
...
export const sendBudgetAlertNotifications = (
  ...
  try {
    ...
    notifiedToday =
      parsed?.date === today ? new Set(parsed.cats || []) : new Set();
  } catch {
  ...
  if (changed) {
    try {
      localStorage.setItem(
        BUDGET_KEY,
        JSON.stringify({ date: today, cats: Array.from(notifiedToday) }),
      );
    } catch {
      /* ignore */
    }
 ...
 */
export const generateRenewalNotifications = (
  subscriptions: Subscription[],
): Array<{
  id: string;
  text: string;
