...
export default function SubscriptionModal({
    ...
    if (subscription) {
      setFormData({
        ...
        billingDay:
          ...
          new Date(subscription.nextDate).getDate() ||
          1,
  ...
  const handleDelete = async () => {
    if (window.confirm(t("sub.delete_confirm"))) {
      debugLog(
        "REMOVE_ACTION",
        `Confirmed delete from Edit Modal for ID: ${subscription.id}`,
      );
      setLoading(true);
      setError(null);
  ...
  const nextPaymentMonth = currentDate.toLocaleString("default", {
    month: "long",
  });

  return (
    <div className="fixed inset-0 z-[70] flex items-start justify-center p-2 sm:items-center sm:p-4 sm:p-6 animate-in fade-in duration-200">
      <div
        className="absolute inset-0 bg-gray-900/60 backdrop-blur-sm transition-opacity"
        onClick={onClose}
      ></div>

      <div className="relative w-[min(100vw-1rem,72rem)] sm:w-full max-w-4xl bg-white dark:bg-gray-900 rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300 border border-gray-100 dark:border-gray-800 max-h-[calc(100dvh-1rem)] sm:max-h-[90vh]">
        {/* Header */}
        <div
        ...
        <div className="flex flex-col gap-4 px-4 py-4 border-b border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 sticky top-0 z-10 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          <div className="flex min-w-0 items-center gap-4">
            <div className="relative group">
              <div
                className="absolute -inset-0.5 rounded-xl opacity-30 blur-sm transition duration-300"
                style={{ backgroundColor: accentColor }}
              ></div>
              <BrandIcon
                type={subscription.type}
                logo={subscription.logo}
                className="w-12 h-12 rounded-xl shadow-sm relative bg-white dark:bg-gray-800"
              />
            </div>
            <div>
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          handleChange("billingDay", parseInt(e.target.value))
                        }
              ...
              <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
                ...
                <div className="grid grid-cols-7 gap-1 text-center text-xs mb-4">
                  {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => (
                    <div
                      key={i}
                      className="text-gray-400 dark:text-gray-500 font-medium py-1"
                    >
                      {d}
                    </div>
                  ))}
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-50 dark:border-gray-700">
                  ...
                  <button
                    onClick={() =>
                      handleChange("reminderEnabled", !formData.reminderEnabled)
                    }
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors`}
                    style={{
                      backgroundColor: formData.reminderEnabled
                        ? accentColor
                        : "#e5e7eb",
                    }} // Keep light gray track for off state visually
                  >
                    <span
              ...
              {subscription.history && subscription.history.length > 1 && (
                <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
                  <h3 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                    ...
                    {t("modal.payment_history")}
                  </h3>
                  <div className="flex items-end gap-1 h-14">
                    {subscription.history.slice(-12).map((amount, i, arr) => {
                      const maxVal = Math.max(...arr);
                      const height =
                        maxVal > 0 ? Math.max(10, (amount / maxVal) * 100) : 10;
                  ...
                  <p className="text-[10px] text-gray-400 mt-2">
                    {subscription.history.length} {t("modal.payments_recorded")}
                  </p>
                </div>
              )}
