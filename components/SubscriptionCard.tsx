...
export default function SubscriptionCard({
  ...
  const defaultTierIndex = useMemo(() => {
    ...
    const lowest = monthlyTiers.reduce(
  ...
  const handleSave = async () => {
    ...
    if (
      existingSubscriptions.some(
        (sub) => sub.name.toLowerCase() === service.name.toLowerCase(),
      )
    ) {
    ...
    try {
      ...
      const selectedTier =
        selectedTierIndex >= 0 ? tiers[selectedTierIndex] : null;
  ...
  return (
    <div className="relative w-[min(100vw-1rem,28rem)] sm:w-full max-w-md mx-auto">
      <div className="relative bg-white dark:bg-gray-900 rounded-[24px] shadow-2xl overflow-hidden border border-gray-100 dark:border-gray-800 flex flex-col max-h-[calc(100dvh-1rem)] sm:max-h-[85vh]">
        {/* Decorative header gradient */}
        <div
          className="absolute top-0 left-0 right-0 h-32 opacity-10 pointer-events-none"
          style={{
            background: `linear-gradient(180deg, ${brandColor}, transparent)`,
          }}
        />
        ...
        <div className="overflow-y-auto flex-1 custom-scrollbar">
          ...
          <div className="relative z-10 flex flex-col items-center px-5 pb-4 pt-10 text-center sm:px-8">
            <motion.div
            ...
            >
              ...
              {/* Glow behind logo */}
              <div
                className="absolute inset-0 bg-current blur-2xl opacity-20 rounded-full -z-10 transform scale-125 transition-transform group-hover:scale-150"
            ...
            {/* Company Info Row */}
            <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-[11px] text-gray-400 mt-1.5 mb-1">
            ...
            <div className="px-6 pb-3 pt-1">
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                ...
                {fact(
                  TrendingUp,
                  t("card.value"),
                  service.netWorth || service.globalUserCount,
                )}
              </div>
            </div>
          )}
                  ...
                  {tiers.map((tier, idx) => {
                    ...
                    return (
                      <motion.button
                        ...
                        className={`relative py-3 px-2.5 rounded-xl text-center transition-all border-2 ${
                          isSelected
